...
gulp.task('build', ['templates', 'lint', 'static'], function () {
    return gulp.src('app/components/app/app.js').pipe($.browserify({
        insertGlobals: true,
...
gulp.task('html', function () {
    return gulp.src('app/**/*.html').pipe($.html2js({
        ...
        useStrict: true
    })).pipe($.concat('templates.js')).pipe(gulp.dest('./scratch'));
});
...
gulp.task('templates', function () {
    return gulp.src(['app/**/**.html']).pipe($.ngTemplates({
        filename: 'templates.js',
        module: 'app.templates',
        path: function path(_path, base) {
            return _path.replace(base, '');
        }
